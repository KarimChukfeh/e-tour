...
contract TicTacChain is ETour {
    ...
    function _registerTicTacChainTiers() internal {
        ...
        // ============ Tier 4: 64-Player Pro ============
        uint8[] memory tier4Prizes = new uint8[](64);
        ...
        // ============ Tier 5: 128-Player Pro (Championship) ============
        uint8[] memory tier5Prizes = new uint8[](128);
        tier5Prizes[0] = 30;   // 1st
        tier5Prizes[1] = 18;   // 2nd
        tier5Prizes[2] = 12;   // 3rd
        tier5Prizes[3] = 10;   // 4th
        tier5Prizes[4] = 8;    // 5th
        tier5Prizes[5] = 7;    // 6th
        tier5Prizes[6] = 5;    // 7th
        tier5Prizes[7] = 4;    // 8th
        tier5Prizes[8] = 3;    // 9th
        tier5Prizes[9] = 3;    // 10th
        // 11th-128th: 0%
        for (uint8 i = 10; i < 128; i++) {
            tier5Prizes[i] = 0;
        }
        ...
        // ============ Tier 6: 2-Player Pro (High Stakes) ============
        uint8[] memory tier6Prizes = new uint8[](2);
        tier6Prizes[0] = 100;  // 1st: 100%
        tier6Prizes[1] = 0;    // 2nd: 0%
        
        _registerTier(
            6,                              // tierId
            2,                              // playerCount
            12,                             // instanceCount
            0.0015 ether,                   // entryFee
            Mode.Pro,                       // mode (with blocking)
            DEFAULT_ENROLLMENT_WINDOW,
            DEFAULT_MATCH_MOVE_TIMEOUT,
            DEFAULT_ESCALATION_INTERVAL,
            tier6Prizes
    ...
    function forceEliminateStalledMatch(
        ...
        require(matchData.status == MatchStatus.InProgress, "Match not active");
        require(block.timestamp >= matchData.timeoutState.escalation2Start, "Tier 2 not reached");
        require(
        ...
        matchData.isTimedOut = true;
        matchData.timeoutClaimant = msg.sender;
        matchData.timeoutState.activeEscalation = EscalationLevel.Escalation2_AdvancedPlayers;
        matchData.timeoutState.timeoutActive = true;

        uint256 entryFee = _tierConfigs[tierId].entryFee;
        playerForfeitedAmounts[tierId][instanceId][matchData.player1] += entryFee;
        playerForfeitedAmounts[tierId][instanceId][matchData.player2] += entryFee;

        emit PlayerForfeited(tierId, instanceId, matchData.player1, entryFee, "Tier 2: Eliminated by advanced player");
        emit PlayerForfeited(tierId, instanceId, matchData.player2, entryFee, "Tier 2: Eliminated by advanced player");
    ...
    function claimMatchSlotByReplacement(
        ...
        require(matchData.status == MatchStatus.InProgress, "Match not active");
        require(block.timestamp >= matchData.timeoutState.escalation3Start, "Tier 3 timeout not reached");
        ...
        if (isEnrolled[tierId][instanceId][msg.sender]) {
            ...
            for (uint8 r = 0; r <= currentRound; r++) {
                for (uint8 m = 0; m < rounds[tierId][instanceId][r].totalMatches; m++) {
                    ...
                    Match storage checkMatch = matches[checkMatchId];

                    if (checkMatch.status == MatchStatus.InProgress &&
                        (checkMatch.player1 == msg.sender || checkMatch.player2 == msg.sender)) {
                        revert("Cannot use Tier 3 while actively playing in this tournament");
                    }

                    if (r == currentRound &&
                        checkMatch.status == MatchStatus.Completed &&
                        checkMatch.winner == msg.sender) {
        ...
        matchData.isTimedOut = true;
        matchData.timeoutClaimant = msg.sender;
        matchData.timeoutState.activeEscalation = EscalationLevel.Escalation3_ExternalPlayers;
        matchData.timeoutState.timeoutActive = true;
        ...
        emit PlayerForfeited(tierId, instanceId, matchData.player1, entryFee, "Tier 3: Replaced by external player");
        emit PlayerForfeited(tierId, instanceId, matchData.player2, entryFee, "Tier 3: Replaced by external player");
    ...
    function _isPlayerInAdvancedRound(
        ...
        Round storage stalledRound = rounds[tierId][instanceId][stalledRoundNumber];

        for (uint8 m = 0; m < stalledRound.totalMatches; m++) {
            bytes32 matchId = _getMatchId(tierId, instanceId, stalledRoundNumber, m);
            Match storage matchData = matches[matchId];
    ...
    function _completeMatchByReplacement(
        ...
        matchData.status = MatchStatus.Completed;
        matchData.winner = replacementPlayer;
        matchData.isDraw = false;
